...
class Camera(object):
    ...
    def connect(self):
        ...
        config.enable_stream(
            rs.stream.depth, self.im_width, self.im_height, rs.format.z16, self.fps
        )
        config.enable_stream(
            rs.stream.color, self.im_width, self.im_height, rs.format.bgr8, self.fps
        )
    ...
    def get_data(self):
        ...
        depth_image = np.asanyarray(
            aligned_depth_frame.get_data(), dtype=np.float32)
